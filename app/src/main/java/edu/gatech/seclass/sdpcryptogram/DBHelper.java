...
//            }
      ...
      public String getlastUserText(String username,int cryptID) {
        String strgetLastUserText = "SELECT " + PLAYER_GAMES_TEXT + " FROM " + TABLE_PLAYER_GAMES + " WHERE " +
                PLAYER_GAMES_PLAYER_USERNAME + " IN ('" + username + "') AND " +
                PLAYER_GAMES_CRYPTOGRAM_ID + " = " + Integer.toString(cryptID) + " ;";
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor getLastUserText_cursor = db.rawQuery(strgetLastUserText, null);
        int count = getLastUserText_cursor.getCount();
        String[] strUserText = new String[count];
        String strText = "";
        int i = 0;
        if (getLastUserText_cursor.moveToFirst()) {
            do {
                strUserText[i] = getLastUserText_cursor.getString(0);
                i = i + 1;

            }
            while (getLastUserText_cursor.moveToNext());
            {

            }
            strText = strUserText[count-1];
        }
    ...
    * @param player_username  = the username used to log into the application
    ...
    /**
     * Retrieve single user activity_user_rating
     * @param username=the username used to log into the application
     */
    public  String[] displaySingleUserRatings(String username)
    {
        ...
        String[] ratinglist = {"Correct: 0","Incorrect: 0","Started: 0"};
        ...
        if(userratings_cursor.moveToFirst())
        {
                ...
                if(status == 'C')
                {
                    ...
                    sbc.append("Correct: ");
                    sbc.append(Integer.parseInt(userratings_cursor.getString(1)));
                    ratinglist[0] = sbc.toString();
                }
                else if(status == 'I')
                {
                    StringBuffer sbi = new StringBuffer();
                    sbi.append("Incorrect: ");
                    sbi.append(Integer.parseInt(userratings_cursor.getString(1)));
                    ratinglist[1] = sbi.toString();
                }
                else if(status == 'S')
                {
                    StringBuffer sbs = new StringBuffer();
                    sbs.append("Started: ");
                    sbs.append(Integer.parseInt(userratings_cursor.getString(1)));
                    ratinglist[2] = sbs.toString();
                }
    ...
    /*
     * int[3]: correct, incorrect, started
     *
     */
    public int[] RetrieveUserRating(String username) {
        HashMap<String,Integer> ratings = new HashMap<>();
        ratings.put("Correct",0);
        ratings.put("Incorrect",0);
        ratings.put("Started",0);
        String[] ratingStr = displaySingleUserRatings(username);
        if(ratingStr != null) {

            for (int i = 0; i < ratingStr.length; i++) {
                int index = ratingStr[i].indexOf(":");
                String key = ratingStr[i].substring(0,index);
                int value =Integer.parseInt(ratingStr[i].substring(index+2,ratingStr[i].length()));
                ratings.put(key,value);
            }
        }

        int[] playerrating  ={ratings.get("Correct"),ratings.get("Incorrect"),ratings.get("Started")};
        return playerrating;
    }


    /**
     * Retrieve  activity_user_rating of all users
     * TODO
     */
    public  String[] displayAllUserRatings() {
        ...
        {
            System.out.println("Error creating table temp1 for activity_user_rating");
        }
        try
        {
            db.execSQL(sb2.toString());
        ...
        {
            System.out.println("Error creating table temp2 for activity_user_rating");
        }
        try
        {
            db.execSQL(sb3.toString());
        ...
        {
            System.out.println("Error creating table temp3 for activity_user_rating");
        }
